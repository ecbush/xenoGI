...
class Genome:
    ...
    def __init__(self, geneL, seqD, params ):
...
object and list of events.'''
        ...
        return Genome(geneL, seqD, self.params),geneCounter,logL

    def deletion(self,geneL,seqD):
        '''Delete genes.'''
        logL=[]
        if random.random() < self.params.delProb:
            delSize = random.choice(range(self.params.minDel,self.params.maxDel+1))
    ...
    def duplication(self,geneL,seqD,geneCounter):
        ...
        logL=[]
        if random.random() < self.params.dupProb:
            dupSize = random.choice(range(params.minDup,params.maxDup+1))
    ...
    def inversion(self,geneL):
        ...
        logL=[]
        if random.random() < self.params.invProb:
            invSize = random.choice(range(params.minInv,params.maxInv+1))
    ...
    def xtrans(self,geneL,seqD,geneCounter,aaFreq):
        ...
        '''
        logL=[]
        if random.random() < self.params.hgtProb:
            hgtSize = random.choice(range(self.params.minHgt,self.params.maxHgt+1))
            ...
            for i in range(hgtSize):
                newGenesL.append(geneCounter)
                seqLen = random.choice(range(self.params.minSeqLen,self.params.maxSeqLen))
...
self.params.model. This stuff based on Spielman's suggestions on how
to use pyvolve machinery.
        ...
        if random.random() < params.smallIndelProb:
            size = random.choice(range(self.params.minIndel,self.params.maxIndel+1))
            if random.random() < 0.5:
                # del
                start = random.choice(range(len(seq)-self.params.maxIndel))
...
eovlution model in self.params.model.
        '''
        for gene in geneL:
...
if __name__ == "__main__":
    ...
    f = pyvolve.EmpiricalModelFrequencies(params.model)
